...
- Create an output directory with images and time surfaces
"""
import argparse
import cv2
import numpy as np
...
def convert_images(images, out_dir):
...
if __name__ == "__main__":
    ...
    if helpers.check_already_exists(os.path.join(args.out_dir, "frames")):
        print("Image files in for sequence", os.path.basename(args.out_dir), "already exist and will not be overwritten. Please delete them manually to re-run frame conversion.")
    ...
    event_time_range, images, image_timestamps, _ = data_io.load_dataset(args.dataset, args.in_dir, required_data)
    ...
    convert_images(images, args.out_dir)
